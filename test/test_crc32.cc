...
class crc32 : public ::testing::TestWithParam<crc32_test> {
public:
    void Hash(crc32_test param, crc32_func crc32, bool works_with_initial = true) {
        uint32_t crc = 0;
        if (!works_with_initial && param.crc != 0) {
            GTEST_SKIP();
            return;
        }
        if (param.buf != NULL) {
...
INSTANTIATE_TEST_SUITE_P(crc32, crc32, testing::ValuesIn(tests));
...
uint32_t crc32_pclmulqdq(uint32_t crc, const uint8_t *buf, uint64_t len) {
    ALIGNED_(16) crc32_fold crc_state;
    uint64_t safe_buf_size = len + (len % 128);
    ALIGNED_(16) uint8_t *safe_buf = (uint8_t *)calloc(1, safe_buf_size);
    if (safe_buf == NULL)
        return 0;
    ...
    crc32_fold_copy_pclmulqdq(&crc_state, safe_buf, buf, len);
    crc = crc32_fold_final_pclmulqdq(&crc_state);
    free(safe_buf);
    return crc;
}
TEST_P(crc32, pclmulqdq) {
    if (!x86_cpu_has_pclmulqdq) {
        GTEST_SKIP();
        return;
    }
    Hash(GetParam(), crc32_pclmulqdq, false);
}
#endif