    ...
    *destLen = 0;
    ...
    do {
        if (stream.avail_out == 0) {
            stream.avail_out = left > UINT_MAX ? UINT_MAX : (unsigned int)left;
            left -= stream.avail_out;
        }
        if (stream.avail_in == 0) {
            stream.avail_in = sourceLen > UINT_MAX ? UINT_MAX : (unsigned int)sourceLen;
...
unsigned long Z_EXPORT PREFIX(compressBound)(unsigned long sourceLen) {
    ...
    return bound > ULONG_MAX ? ULONG_MAX : (unsigned long)bound;
}
